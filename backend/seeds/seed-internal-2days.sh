...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
    echo "Usage: ./seed-internal-2days.sh <base_url> <friendly_id> <password>"
    echo "Example: ./seed-internal-2days.sh http://localhost:3000 admin password123"
    exit 1
fi
...
PRE_SURVEY_OPEN_DATE="2025-08-18T00:00:00Z"
PRE_SURVEY_CLOSE_DATE="2025-08-26T00:00:00Z"
POST_SURVEY_OPEN_DATE="2025-10-23T00:00:00Z"
POST_SURVEY_CLOSE_DATE="2025-11-01T00:00:00Z"
...
entertainment_data=$(cat data/entertainment-market-template.json | jq -r '.[] | "\(.topic)|\(.question)"')
climate_data=$(cat data/climate-market-PILOT.json | jq -r '.[] | "\(.topic)|\(.question)"')
...
# Create 30 experiment (climate) markets - every 2 days
for i in {1..30}; do
    ...
    echo "Creating experiment market $i: $topic (opens: $open_date, closes: $close_date)"
